    ...
    async (value?: string | number) => {
      ...
      if (value) {
        ...
        form.change(
          'signers',
          storeSigners
            ?.map((signer:any, i:any) => ({
              ...signer,
              role: undefined,
              order: i,
            }))
            .filter((s:any) => s.name || s.email || s.order === 1),
        );
      }
    },
    [
  ...
  const handleDocumentPrepare = useCallback(
    async (values: DocumentValues) => {
      ...
      if (!_.isEqual(values, initialValues)) {
        const document:any = await updateDocument({
          values: {
        ...
        const actualSigners = document.signers.filter(
          (signer:any) => isOnlyMeType || !signer.isPreparer,
        );
                ...
                {isTemplate ? (
                  <>
                    ...
                    <FieldArray
                      ...
                      isItemDeletablePredicate={({ fields }:any) => fields.length > 1}
                    />
                  </>
                ) : (
                  <>
