...
function* handleTeamMembersGet({payload, meta,}: ReturnType<typeof getTeamMembers.request>) {
...
function* handleDeleteTeamMembers({
    ...
    const {teamMembersIds}: any = payload;
...
function* handleToAdminUpgrade({
    ...
    //@ts-ignore
        ...
        yield put(upgradeToAdmin.failure(error, meta));
    } finally {
        if (yield cancelled()) {
...
function* handleToUserDonwgrade({
    ...
    //@ts-ignore
        ...
        yield put(downgradeToUser.failure(error, meta));
    } finally {
        if (yield cancelled()) {
